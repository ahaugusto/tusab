...
export default function ResetModal({ open, darkMode, onClose, onResetDone }) {
  ...
  return (
    <AnimatePresence>
      {open && (
        <ModalWrapper key="reset-modal" onClose={handleClose} zIndex="z-[9999]" backdrop="bg-black/55" label={t('reset.title')}>
          <motion.div initial={{ scale: 0.95, y: 12 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 12 }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className={`text-sm font-bold ${darkMode ? 'text-white' : 'text-slate-900'}`}>{t('reset.title')}</h3>
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder={t('reset.confirm_word')} autoFocus
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                disabled={confirmText !== t('reset.confirm_word') || resetando}
                onClick={handleReset}
