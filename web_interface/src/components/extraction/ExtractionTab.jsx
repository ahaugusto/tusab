            ...
            <div className={`mx-4 mt-3 flex items-center gap-1 p-1 rounded-xl border ${darkMode ? 'bg-white/3 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-[11px] font-bold transition-colors ${BTN_FOCUS}
                  ${fontePreSelecionada === 'youtube' ? 'bg-primary-button text-white shadow-sm' : darkMode ? 'text-slate-400 hover:text-slate-200' : 'text-slate-500 hover:text-slate-700'}`}>
                🎬 {t('extraction.source_youtube')}
              </button>
              <button
                onClick={() => setFontePreSelecionada('fonte-publica')}
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-[11px] font-bold transition-colors ${BTN_FOCUS}
                  ${fontePreSelecionada === 'fonte-publica' ? 'bg-primary-button text-white shadow-sm' : darkMode ? 'text-slate-400 hover:text-slate-200' : 'text-slate-500 hover:text-slate-700'}`}>
            ...
            {fontePreSelecionada !== 'youtube' ? (
              ...
              <div className="flex gap-4 items-stretch">
                ...
                <div className="flex-1 space-y-2">
                  <p className={`text-[10px] font-bold uppercase tracking-widest ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>{t('channel.title')}</p>
                  <CanalUrlSearchInput
                    darkMode={darkMode}
                    value={canalInput}
              ...
              <div className="space-y-2">
                <p className={`text-[11px] font-bold uppercase tracking-widest ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>{t('channel.title')}</p>
                <CanalUrlSearchInput
                  darkMode={darkMode}
                  value={canalInput}
                  onChange={v => { setCanalInput(v); setCanalError(''); }}
            ...
            <button onClick={handleStart} disabled={!canalConfigurado && !isRunning && !regras.fontes_publicas}
              className={`ml-auto flex items-center gap-1.5 px-5 py-2 rounded-xl text-xs font-bold transition-all active:scale-[0.98]
                ...
                ${isRunning
                  ...
                  : 'bg-primary-button text-white hover:bg-primary-button/85 shadow shadow-primary-button/20'} ${BTN_FOCUS}`}>
