...
**Criado:** 03/jul/2026 · **Atualizado:** 30/ago/2026 (achados de contraste e foco, levantamento por grep repetido) · **Fonte da verdade:** este documento reflete o código em produção (levantamento por grep em `web_interface/src/`). Divergência entre doc e código = bug de documentação; abrir correção. Esta é a cópia oficial versionada — existe um espelho de trabalho em `Documentação do Produto/Design System — Tusab.md` (fora do git); ao editar um, replicar no outro.
...
| `text-slate-400` como texto sozinho sobre fundo claro (`bg-white`/`bg-slate-50`) | Contraste real ~2.5:1 — falha WCAG AA (mín. 4.5:1). Achado real em `ChatDrawer.jsx` (ago/2026): "Não indexado", contador de progresso, texto de log e timestamp de log, todos abaixo do piso. `text-slate-500` é o piso mínimo confirmado (4.55–4.76:1) para esse par de fundo — usar como padrão, não `slate-400`, sempre que o texto secundário estiver sozinho (sem peso/tamanho maior compensando) |

---

## 11. Inventário completo de moléculas e organismos

O mapa de TODA a interface (shell, 14 modais, chat completo, feedback global, estados, formulários, dados, superfícies especiais) está em **[inventario-componentes.md](inventario-componentes.md)** — incluindo as dívidas de consistência encontradas no levantamento.
