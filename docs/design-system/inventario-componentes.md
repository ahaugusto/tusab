...
**Criado:** 03/jul/2026 · **Atualizado:** 30/ago/2026 · Complementa [design-system.md](design-system.md) (tokens/fundações). Este documento mapeia **moléculas e organismos** — o que existe de fato em `web_interface/src/components/` (37 arquivos, ~12.500 linhas).
...
| `ChatDrawer.jsx` (2.376+ linhas, maior organismo) nunca importa `BTN_FOCUS` | 143 usos de `BTN_FOCUS` em 18 arquivos do projeto, mas zero no ChatDrawer — inclusive o botão de enviar mensagem, provavelmente o controle mais usado do app, sem focus ring próprio (só o `:focus-visible` genérico do `index.css`). Achado real, ago/2026 | 🔧 Corrigido (ago/2026): `BTN_FOCUS` importado e aplicado no botão de enviar e nos demais controles interativos do arquivo |
| Prop `btnFocus` com fallback `''` (string vazia) em vez de `BTN_FOCUS` | Em `EmbeddingsStatus.jsx`, se o pai não passar a prop, o foco visível desaparece silenciosamente — contrato de prop que finge ter default seguro mas não tem | Revisar todo componente que recebe `btnFocus` como prop — fallback deve ser sempre `= BTN_FOCUS`, nunca string vazia |
| `text-slate-400` como texto secundário sozinho sobre fundo claro | Contraste real ~2.5:1 em `ChatDrawer.jsx` (modal "Base de conhecimento": label "Não indexado", contador de progresso, texto/timestamp de log) — falha WCAG AA. Achado real via medição, ago/2026, ver §10 de design-system.md | 🔧 Corrigido (ago/2026): trocado por `text-slate-500` (4.55–4.76:1) nos 5 pontos identificados |
