...
- **Job `tests` do pipeline de release nunca conseguiria rodar** no runner self-hosted macOS recém-registrado — `runs-on` pedia label `ARM64` e o job baixava um Python compilado só para `aarch64-apple-darwin`, mas o Mac físico disponível (MacBookPro11,1) é Intel. Corrigido pra `X64` + Python `x86_64-apple-darwin` nesse job específico, que não depende de arquitetura (só roda pytest e build do frontend). Os jobs que de fato exigem Apple Silicon (`build-macos`, `electron-notarize` — geram o instalador assinado) permanecem `ARM64`, mantendo a decisão já registrada de publicar instalador macOS só para Apple Silicon.
