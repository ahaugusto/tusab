...
- **Job `tests` do pipeline de release estava preso a um runner self-hosted macOS que não existe mais funcional** — `runs-on` pedia label `ARM64`, mas o único Mac físico disponível (MacBookPro11,1) é Intel; ajustar só o label não bastava, porque o PyTorch não publica wheel para macOS Intel desde a versão 2.2.2 (`requirements.txt` exige `torch>=2.12.1`) — esse job nunca teria hardware self-hosted elegível. Investigado o motivo original de ter saído do `ubuntu-latest` (bloqueio de billing reportado em meados de 2026) e confirmado que não reproduz mais — job voltou pro `ubuntu-latest` hospedado, mais barato e sem depender de nenhum Mac.
...
- **`build-macos` gera o instalador Apple Silicon a partir de um Mac Intel (cross-build), sem depender de hardware/aluguel ARM64** — a suposição de que gerar um `.dmg`/`.zip` arm64 exigia rodar em hardware arm64 estava errada: `electron-builder --arm64`, `codesign` e a notarização (upload pra Apple) manipulam os binários como arquivo, sem executar código arm64. Só a instalação de dependências Python precisava de ajuste — trocado o download-e-execução de um interpretador Python compilado pra `aarch64-apple-darwin` por `pip install --platform macosx_14_0_arm64 --only-binary=:all:` (baixa os wheels arm64, inclusive torch, sem nunca rodar um binário arm64 no host Intel). Validado via 3 rodadas de `workflow_dispatch` no runner self-hosted (`tusab-macos-runner`, MacBookPro11,1): binário do Electron e Python embutido confirmados via `file` como `Mach-O 64-bit executable arm64`. Ajustes de compatibilidade descobertos na primeira release de verdade: `--break-system-packages` no pip (necessário quando o Python do runner é gerenciado por Homebrew/PEP 668) e remoção do `pip install --upgrade pip` (falha em Python instalado via Homebrew, sem RECORD file pra desinstalar). A v1.0.42 em si acabou publicada via `macos-latest` hospedado — o self-hosted travou 2x nesta mesma release por suspensão do Mac e configuração de sudo, ambos fora do escopo de código; `runs-on` volta pro self-hosted a partir da próxima release.
