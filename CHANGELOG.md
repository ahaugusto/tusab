...
- **Requisito mínimo de macOS corrigido no README** — o instalador Apple Silicon exige **macOS 14 (Sonoma) ou mais novo**, não macOS 12+ como estava documentado.
...
- Teste da área de Antropologia desatualizado desde a adição do módulo Wiktionary causava falha em 100% das execuções de CI desde 01/ago; corrigido, sem relação com nenhum bug real do produto.
...
- Job `tests` do pipeline de release migrado de volta pro `ubuntu-latest` hospedado (mais barato, sem depender de runner próprio); descoberto no processo que o piso `torch>=2.12.1` do `requirements.txt` é o que fixa o requisito de macOS 14+ citado acima.
- `build-macos` passa a suportar cross-build de ARM64 a partir de host Intel (`pip install --platform macosx_14_0_arm64` em vez de baixar-e-executar um interpretador arm64) — permite gerar o instalador Apple Silicon sem depender de hardware ARM64 dedicado.
