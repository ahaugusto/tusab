...
>
> **Lição sobre tabelas de comparação (10/ago/2026):** corrigi só a linha "Reset total" de `perfis-e-jornadas.md` numa primeira passada — o usuário depois apontou que "Painel Admin" também estava errado pro Pesquisador (tabela dizia "—", código real `usePerfil.js::PERFIS_CONFIG.pesquisador.admin = true`). Ao verificar TODA a tabela célula por célula contra `PERFIS_CONFIG`, achei mais 2 erros que uma checagem pontual não pegaria: "Visão Geral" errado pro Professor, "Gerenciar repositório" com uma coluna a menos que devia (a ação real é gated só por `readonlyMap` por-projeto, não por perfil — `deletar_arquivos`/`limpar_canal` no `PERFIS_CONFIG` são flags mortas, nunca lidas pelo frontend). **Regra prática: numa tabela de comparação por perfil/flag, sempre verificar TODA linha contra o código, nunca só a linha que motivou a revisão** — um erro encontrado é sinal de auditoria incompleta, não de correção pontual suficiente.
>
> **Feature subdocumentada (10/ago/2026):** busca em fontes públicas (perfil Pesquisador) é descrita nas páginas como só "arXiv, OpenAlex, DOAJ, Zenodo, Crossref, FHIR" — mas `tests/test_fontes_*.py` (direito, economia, fisica, patrimonio, saude, tecnologia, terra, antropologia) sugere uma superfície bem maior, registrada em `tusab_engine/motor/fontes/`. Antes de escrever qualquer coisa sobre isso, ler `router_fontes.py` (endpoint `GET /fontes` deve listar todas as áreas/fontes registradas) — não confiar na lista já escrita nas páginas atuais, ela é exatamente o que está desatualizado.
