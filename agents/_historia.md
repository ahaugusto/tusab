...
**Área 4 (Direito, normas, legislação e governo), 28/jul/2026.** Senado Federal tem dois endpoints: o documentado (`materia/pesquisa/lista`) está oficialmente depreciado desde 18/mar/2025 com desativação completa marcada pra 01/fev/2026 (já passou, mas ainda responde); o substituto oficial (`/dadosabertos/processo`) **não filtra por palavra-chave de verdade** — testado ao vivo com termo comum ("inteligência artificial") e termo raro ("xenotransplante"), ambos retornaram o mesmo total de 10.702 registros, confirmando que o parâmetro é ignorado. Senado descartado. **Câmara dos Deputados** confirmada como único candidato real — e com um achado importante: diferente do Datajud (rejeitado antes por só ter metadado processual, sem texto integral), a Câmara expõe `urlInteiroTeor` — um PDF real com o texto completo da proposição. Testado ao vivo: um PL sobre regulação de IA virou 7 páginas de texto extraído via `pdfplumber` (mesmo padrão de `arxiv.py`). Implementado como busca em 3 chamadas por item (busca → detalhe → PDF), best-effort — se qualquer etapa falhar, cai pra ementa sozinha, nunca derruba o item. Confirma que a rejeição do Datajud foi específica ao Datajud, não generalizável a "toda fonte jurídica brasileira".
