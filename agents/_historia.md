...
Implementação: `regras.arxiv = false` em `usePerfil.js` (antes `true`) — oculta o toggle YouTube/arXiv em `ExtractionModal`/`ExtractionTab`, o card "Pesquisa acadêmica" na Home e a menção no onboarding do Pesquisador (`Onboarding.jsx::s3Body()`, agora checa `PERFIS_CONFIG.pesquisador.arxiv`), sem apagar nenhum código — `tusab_engine/motor/arxiv.py`, os endpoints `/arxiv/*` e os filtros de data/autor continuam intactos e testados. FAQ do Electron (`help.html`, 3 idiomas) comentado com `<!-- -->`, não removido. Reversível em um flip de flag quando o Tusab Saúde herdar essa base — **não repropor arXiv/FHIR pro Pesquisador B2C sem essa decisão ser revisitada primeiro.**
