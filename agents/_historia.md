...
### Nomenclatura do produto: "Agente" → "Assistente" no front-end, backend mantém "agent" (07/ago/2026)
Augusto questionou o termo "Agente" (aba/feature de chat RAG) — em 2026 "AI agent" carrega peso técnico específico (autonomia, tool-calling, planejamento multi-passo) que o Tusab não tem: o pipeline é BM25 → CrossEncoder → prompt com persona → LLM → checagem de alucinação, sem loop autônomo nem chamada de ferramenta por iniciativa própria. Decisão: **termo único "Assistente"/"Assistant"/"Asistente"**, aplicado em tudo que é lido por humano — os 3 `locales/*.json` (chaves E valores, ex.: `agent.chat_title` → `assistente.chat_title`, "Chat com o Agente" → "Chat com o Assistente"), nomes de arquivo/pasta do frontend (`AgentTab.jsx` → `AssistenteTab.jsx`, `useAgentConfig.js` → `useAssistenteConfig.js`, `components/agent/` → `components/assistente/`).
**Escopo explicitamente excluído (decisão do Augusto — "itens que podem gerar regressão não precisam ser renomeados"):** o pacote Python `tusab_engine/agent/`, o arquivo `agent_config.json` persistido em disco nas máquinas já instaladas, as rotas `/agent/*` (espalhadas por `router_agent.py`, `router_estudo.py`, `router_status.py`, `router_digest.py`), o shim `agent_tusab.py`, os testes, o MCP server, e as funções de `services/api.js` que mapeiam 1:1 pras rotas (`saveAgentConfig`, `loadAgentConfig`, `testAgentKey`, `fetchAgentStatus` etc.) — renomear qualquer um desses exige migração de dados de usuários já instalados ou refactor em cascata, sem ganho de UX. **Mesmo padrão já validado no projeto** para o slug `profissional`/label "Especialista" (ver `usePerfil.js`): identificador interno intacto, só o que o usuário lê muda.
**Também fora do escopo, por ser conceito diferente:** a pasta `agents/` na raiz e os slash commands (`/qa`, `/backend`, `/frontend` etc.) são os agentes especializados do Claude Code usados para construir o Tusab — não têm relação com a feature de chat do produto, e a palavra "agente" ali está correta como está.
**Para quem for tocar no código:** ver `código internamente ainda usa "agent"` na tabela de decisões técnicas não-óbvias do `CLAUDE.md` antes de "corrigir" uma referência a `agent_config.json`/`/agent/*`/`tusab_engine/agent/` pensando que é resíduo esquecido do rename — é proposital.
...
**Resolvido (07/ago/2026)**: `tusab` e `tusab-public` consolidados num único repo público. Antes do flip de visibilidade, Augusto notou um problema real que a recomendação original (28/jul) não previa: `Documentação do Produto/` (Plano B2B, roadmap, propostas de expansão de mercado) estava versionada dentro do `tusab` — tornar o repo público exporia estratégia de negócio inteira, não só código. Resolvido em duas etapas: (1) `git filter-repo --path "Documentação do Produto/"` extraiu a pasta com histórico completo (91 commits, 30 arquivos) pro novo repo privado `ahaugusto/tusab-docs`; (2) `git filter-repo --path "Documentação do Produto/" --invert-paths` purgou a pasta de **todos** os commits do `tusab` (565→534 commits, os ~30 que só tocavam a pasta foram descartados por ficarem vazios), força-pushed por cima do histórico antigo (tags incluídas). Só depois disso `tusab` foi tornado público (`gh api -X PATCH .../visibility=public`). `build.publish.repo` do electron-builder migrado pra `tusab`; `tusab-public` **deletado** (não mantido como mirror de transição — poucas instalações existentes, `pode apagar o público, nada que vai impactar o lançamento`, decisão de Augusto). Efeito colateral aceito: instalações da v1.0.41/v1.0.42 feitas via `tusab-public` perdem o aviso de atualização automática silenciosamente (app continua funcionando) até reinstalar manualmente uma vez.
...
### Fidelidade numérica no chat — primeira correção estrutural (29/jul/2026)

Reabre o ponto em aberto do incidente "o que é RAG" (28/jul, já registrado): Augusto reportou ao vivo que uma resposta sobre a Lei nº 14.688/2023 (fonte `senado_leis`) veio com números e datas apagados — "Decreto-Lei nº 1.001, de 21 de outubro de 1969" virou "Decreto-Lei nº., de de outubro de" na resposta do `llama3.2:1b`. Diagnóstico ao vivo confirmou: **não é bug de retrieval** (documento salvo em disco tem os números corretos, conferido diretamente no `.txt`) **nem do filtro anti-alucinação** (`_verificar_alucinacao`/`_calcular_confianca_por_sentenca` — código lido, nenhum dos dois toca no texto da resposta, só computam cobertura de vocabulário). É falha genuína de geração do modelo pequeno ao parafrasear contexto denso em números.

**Achado técnico que corrigiu uma sugestão errada de pesquisa:** um agente de pesquisa em background recomendou usar `options.grammar` (GBNF) do Ollama pra restringir slots numéricos por gramática. Testado ao vivo (`curl` direto no `/api/generate` com `grammar: "root ::= \"42\""`, pedindo um número entre 1-100) — **o Ollama ignorou completamente a gramática**, devolveu um número qualquer. `options.grammar` não é respeitado pela API REST do Ollama nesta versão (0.32.3), mesmo sendo uma alegação plausível (é real no `llama-server` cru do llama.cpp, não necessariamente exposto do mesmo jeito pelo wrapper do Ollama) — **não confiar nesse caminho se reaparecer como sugestão sem reteste**. O mecanismo que de fato funciona, confirmado ao vivo: `format` com JSON Schema (`{"type":"object","properties":{"numero":{"enum":[...]}}}`) — o Ollama respeitou o enum corretamente. Não foi usado na correção final porque exigiria mudar o contrato de resposta de markdown livre pra JSON estruturado, incompatível com o pipeline de streaming/renderização atual.

**Correção implementada** (`tusab_engine/agent/chat.py`): `_tem_lacuna_numerica()` detecta a assinatura textual de número apagado (regex `de\s+de` ou `nº` seguido de pontuação sem dígito — cobre o padrão real observado, não uma tentativa de cobrir toda omissão numérica possível). Quando detectado, `chat()` dispara 1 retry com prompt reforçado ("preserve TODOS os números... nunca substitua por espaço em branco"). Testado ao vivo isoladamente: instrução reforçada sozinha (sem detecção+retry) reduz mas não elimina a perda — dispara e depende de comparar antes/depois pra valer a pena. `_gerar_resposta_llm()` foi extraído do dispatch de provider (antes inline, duplicado em pelo menos 5 pontos de `chat.py`) pra poder ser chamado 2x (original + retry) sem copiar o bloco if/elif de novo — escopo desta correção ficou só em `chat()` (não-streaming); `chat_stream()` continua sem o retry (não dá pra refazer geração no meio de um stream já em curso pro cliente sem quebrar o protocolo já documentado no topo do arquivo).

Validado ao vivo 3x consecutivas contra o caso real (Lei nº 14.688) — sem recorrência do padrão "de de"/"nº." em nenhuma. **Achado paralelo, corrigido na mesma sessão:** em 2 das 3 tentativas o modelo misturou conteúdo de outras leis retornadas pelo BM25 na mesma busca (conflação entre chunks, não invenção — o texto "misturado" era real, só pertencia a outra lei).

### Conflação entre chunks — corrigido (29/jul/2026)

Causa raiz encontrada por inspeção direta do retrieval: `_recuperar_contexto('O que a Lei 14.688 alterou...', 'RAG', n=4, ...)` devolvia 4 chunks — o certo (Lei 14.688, score 6.316) e 3 de leis completamente diferentes (Lei 12.015/2009, Lei 14.344/2022, Lei 13.769/2018, scores 2.6-3.8) que só compartilham vocabulário jurídico genérico ("crimes hediondos", "Código Penal", "Decreto-Lei"). O prompt já rotula cada fonte com `<title>` (`_montar_prompt`), mas a instrução ("se qualquer fonte contiver informação relevante — mesmo parcialmente — USE-A") não impede o modelo pequeno de atribuir conteúdo de uma fonte à entidade perguntada sobre outra.

**Achado bônus que destravou a correção:** `index.py::_parsear_todos_chunks()` (indexação de `documents/`/`texts/`, usada por TODAS as fontes públicas + upload manual + texto colado + scraper de URL) nunca parseava o cabeçalho `TITULO:`/`DATA:`/`URL_ORIGEM:` que `_salvar_documento` (fontes/_base.py) e `router_repositorio.py` sempre gravam — usava o nome de arquivo sanitizado (uuid+underscores) como `titulo`, e `data`/`link` ficavam sempre vazios. Diferente do caminho de indexação de vídeos (`_parsear_chunks`), que já fazia esse parsing corretamente. Corrigido — mesmo padrão de regex já usado no outro caminho. Efeito colateral positivo confirmado ao vivo: a citação "fontes" no chat agora mostra o título humano-legível em vez do nome de arquivo, para qualquer documento não-vídeo.

**Correção do retrieval:** `chat.py::_filtrar_por_identificador()` (S3.4) — quando a pergunta cita um identificador no formato "nº de lei" (regex `\d{1,3}(?:\.\d{3})+`, ex: "14.688"), restringe os resultados aos chunks cujo TÍTULO contém esse mesmo identificador. Só aplica quando há match real; sem match, comportamento inalterado (fallback seguro). Depende da correção do parsing de título acima — sem título limpo, o identificador nunca bateria. Extraída como função própria (não inline) porque BM25 degenera com corpus sintético pequeno (IDF negativo quando um termo aparece na maioria dos poucos documentos), tornando difícil testar o filtro isolado passando pelo pipeline completo — testável direto sem precisar de índice real.
...
**Fase 2 concluída**: `state.stats["canal_nome"]` → `state.stats["projeto_nome"]` — contrato sincronizado front+back num commit só. Achado importante durante o mapeamento: `state.projeto_nome` (atributo top-level, separado) JÁ EXISTIA — mas é write-once (guarda override de nome pro motor de extração antes de iniciar, depois reseta pra `""`), semanticamente diferente de `state.stats["projeto_nome"]` (lido continuamente via polling de `/status`, mostra "o que está ativo agora"). Comentário adicionado em `state.py` documentando a distinção pra não confundir agentes futuros. Backend tocado: `state.py` (definição + log entry), `router_agent.py` (2), `router_exports.py` (4), `router_extraction.py` (2 escritas — resposta de `/set-channel` mantida como `canal_nome` deliberadamente, é endpoint genuinamente sobre configurar canal, só a fonte do valor mudou), `router_fontes.py` (o "hack" em si — agora lê/escreve `projeto_nome` em vez de emprestar `canal_nome`, sem mais desalinhamento semântico), `router_repositorio.py` (2), `router_status.py` (1, reset de idle). Frontend: `App.jsx` (2 leituras de `stats.canal_nome`→`stats.projeto_nome`). Validado ao vivo: `/status` expõe `projeto_nome` sem `canal_nome`; `/set-channel` continua respondendo `canal_nome` (decisão deliberada); busca em fonte pública (Wikipedia) não vaza mais nome de canal e restaura o estado anterior corretamente depois. 187 testes, nenhum quebrado (os que citam `canal_nome` são de contratos diferentes — arquivo de índice, request do chat — fora do escopo desta fase).

**Fase 3 concluída**: variáveis/parâmetros internos de `chat.py`, `router_agent.py`, `index.py`, `mcp_server.py` renomeados pra bater com os parâmetros públicos já migrados. O alias explícito `canal_nome = projeto_nome  # alias interno — não altera o restante do código` (2 ocorrências, `chat()`/`chat_stream()`) virou auto-atribuição redundante (`projeto_nome = projeto_nome`) depois de renomear os usos internos — removido de fato, não só disfarçado. Mesmo padrão achado e corrigido em `_recuperar_contexto`'s `canais_extras` (agora `projetos_extras`, propagado até `mcp_server.py`).

**Achado que quase virou bug (`mcp_server.py`)**: renomear `canal_nome`→`projeto_nome` num call site esqueceu uma segunda referência à variável antiga 3 linhas depois (`'canal': c.get('canal', canal_nome)`) — teria sido `NameError` em produção. Pego só porque cada arquivo foi checado com `grep` pós-edição antes de seguir pro próximo, não confiado no `sed` sozinho — reforça a disciplina de verificar cada arquivo individualmente, mesmo em renomeação aparentemente mecânica.

**Achado de dado persistido, tratado com retrocompatibilidade (não migração forçada)**: o arquivo de índice BM25 (`data/agent_index/{projeto}_index.json`, escrito por `indexar()`) tinha campo `canal_nome` na raiz do JSON — índices de usuários já existentes no disco têm esse formato. Em vez de migrar/reescrever arquivos existentes, os 2 pontos de leitura (`_get_agent_status_uncached()`, `_contar_canais_indexados()`) agora aceitam `data.get('projeto_nome', data.get('canal_nome', fallback))` — índices antigos continuam funcionando até serem reindexados naturalmente, sem script de migração. Validado ao vivo: projeto "RAG" (índice no formato antigo, criado antes desta fase) apareceu corretamente em `/agent/status`; depois de reindexar, o arquivo passou a usar `projeto_nome`.

**Decisões de escopo (o que ficou de fora, deliberadamente)**: campos Pydantic `canal_nome`/`canais_extras` em `router_agent.py` (5 modelos) mantidos como estão — são o shim de compatibilidade que permite o frontend (ainda não migrado, isso é Fase 4) continuar enviando o nome antigo no corpo da requisição; `ResumeConversationRequest`/`InjetarHistoricoRequest` (2 modelos sem par `projeto_nome`) também ficaram de fora — não têm o padrão de alias ainda, adicionar isso é trabalho de Fase 4, acoplado à migração do frontend. Config key `canal_indexado` (`agent_config.json`) e `state.logs[].canal`/`_chat_history` manifest `"canal"` — mantidos como nome de campo, só a variável fonte foi renomeada — são contratos consumidos pelo frontend ainda não migrado. `extraction.py` e as partes de `router_extraction.py` que lidam com metadado real de canal do YouTube (`canal_nome_raw`, `canal_handle`, `AutoUpdateConfigRequest.canal_prefixo` vs. `.projeto_prefixo` — já corretamente modelados como dois conceitos distintos) não foram tocados, por serem genuinamente sobre canal, não projeto.
...
### Seleção de área de conhecimento na busca em fontes públicas — multi-área avaliada e adiada (31/jul/2026)

**Contexto:** durante a reorganização da tela de Extração (mover o seletor de área de conhecimento pra fora da `ExtractionModal`, pro usuário ver o que dá pra buscar antes de abrir a modal — `ExtractionTab.jsx`), Augusto perguntou se dava pra habilitar seleção de **mais de uma** área de conhecimento ao mesmo tempo (ex: Tecnologia + Economia simultaneamente).

**Por que foi adiado, não implementado:** a área hoje é só um **filtro de navegação** até uma base específica — `areaSelecionada` é usada em `ExtractionModal.jsx` pra restringir a lista de fontes mostradas no passo seguinte (`areasFontes[areaSelecionada].fontes`), e a busca real (`buscar_fonte()`/`POST /fonte/buscar`) roda contra **uma única base** escolhida pelo usuário (arXiv, Zenodo, CrossRef etc.), não contra um conjunto de bases. Multi-área só faria sentido se a intenção fosse busca federada (buscar ao mesmo tempo em todas as bases de N áreas, agregando resultados) — isso é uma mudança de escopo bem maior que multiplicar um filtro de UI: precisaria de busca paralela em múltiplas fontes, deduplicação, agregação de resultados e possivelmente rate-limit por fonte.

**Decisão:** não implementar agora. Prioridade é publicar a Fase 8 (macOS). Se a demanda por busca federada entre bases voltar à tona, é feature nova de verdade (não um toggle de multi-select), a ser desenhada com `/backend` (viabilidade de busca paralela) e `/produto` (vale o esforço vs. escolher uma base de cada vez?) antes de qualquer código.

### Corrida de criação de release entre build-windows e build-macos — corrigida na raiz (07/ago/2026)

**Padrão recorrente, não fluke isolado:** os dois builders rodavam em paralelo (`needs: tests` nos dois) e cada um chamava `electron-builder --publish always`, que faz "GET release da tag → se não achar, POST cria". Quando os dois checam "não existe" quase ao mesmo tempo, ambos tentam criar. Aconteceu 3 vezes em 4 releases seguidas, com sintomas diferentes cada vez — o que atrasou o diagnóstico porque parecia bug novo a cada ocorrência:
- **v1.0.42**: os dois `POST` tiveram sucesso — GitHub aceitou dois objetos de release pra mesma tag, assets espalhados entre eles (Windows num, macOS no outro). `gh release view` resolvia ambiguamente pra um dos dois.
- **v1.0.43**: mesmo padrão de dois objetos — corrigido baixando os assets do órfão (`gh api .../releases/{id}/assets`, não a URL de download por tag, que resolve pro objeto errado quando há dois) e subindo pro release "principal", depois apagando o órfão.
- **v1.0.44**: dessa vez o `POST` do build-windows recebeu `422 already_exists` (o macOS já tinha criado primeiro) — e isso **derrubou o job inteiro** em vez de criar um duplicado. O `.exe` já tinha subido antes do erro, mas `latest.yml` e o `.exe.blockmap` ficaram de fora. Corrigido com `gh run rerun --job` só do build-windows (o release já existia, então o rerun só fez upload, sem tentar criar de novo).

**Fix estrutural** (`release.yml`): novo job `create-release` (roda em `ubuntu-latest`, rápido) entre `tests` e os dois builders — gera as notas e cria a release vazia sozinho, sem concorrência. `build-windows` e `build-macos` passam a depender de `[tests, create-release]` em vez de só `tests`. Quando os builders chamam `electron-builder --publish always`, o `GET` sempre acha a release já criada — só fazem upload de asset, nunca tentam `POST` de criação. Elimina a corrida na raiz, não é mais um problema de "quem chega primeiro".

**Como diagnosticar se voltar a acontecer:** `gh api repos/{owner}/{repo}/releases --jq '.[] | select(.tag_name=="vX.Y.Z")'` mostra TODOS os objetos pra uma tag (o `gh release view`/URL de download por tag resolve ambíguo quando há mais de um). Se `gh run view --json jobs` mostrar `conclusion: "failure"` num builder que "pareceu" ter completado (asset principal subiu), procure `422 Unprocessable Entity` + `already_exists` no log do step de publish.
