...
Validado ao vivo 3x consecutivas contra o caso real (Lei nº 14.688) — sem recorrência do padrão "de de"/"nº." em nenhuma. **Achado paralelo, ainda sem correção:** em 2 das 3 tentativas o modelo misturou conteúdo de outras leis retornadas pelo BM25 na mesma busca (conflação entre múltiplos chunks — problema diferente de fidelidade numérica, mais próximo do que `_verificar_alucinacao` deveria pegar mas não pega hoje porque a cobertura de vocabulário de cada lei individualmente ainda bate). Próxima instância da mesma classe de problema a investigar.
