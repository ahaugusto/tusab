...
**Documentação versionada** (30/ago/2026): `docs/design-system/design-system.md` (tokens, tipografia, espaçamento, radius, botões, componentes canônicos, motion, acessibilidade, anti-padrões) e `docs/design-system/inventario-componentes.md` (mapa de moléculas/organismos, dívidas de consistência conhecidas) fazem parte do repositório — consultar **antes** de qualquer mudança visual nova, e atualizar junto de qualquer PR que introduza componente/token/padrão. Existe um espelho de trabalho idêntico em `Documentação do Produto/` (fora do git — ver nota abaixo); ao editar um, replicar no outro.

**Nota sobre `Documentação do Produto/`:** essa pasta (roadmap, B2B, DesignOps, changelogs internos) foi movida em 2026-08-07 pro repositório privado separado `ahaugusto/tusab-docs` — continua presente localmente fora do controle de versão deste repo (`.gitignore`), mas não faz mais parte do histórico do `tusab` (removida retroativamente via `git filter-repo` antes de tornar este repo público). Os dois arquivos de Design System são exceção deliberada: voltaram a ser versionados (cópia em `docs/design-system/`) por não conterem informação estratégica/comercial sensível, ao contrário do resto da pasta (Plano B2B, Roadmap, propostas comerciais). `DesignOps — Processo de Sincronização Figma-Código.md` continua só na pasta privada.
