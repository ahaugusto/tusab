...
**Nota:** a pasta `Documentação do Produto/` (Design System, DesignOps, roadmap, B2B) foi movida em 2026-08-07 pro repositório privado separado `ahaugusto/tusab-docs` — continua presente localmente fora do controle de versão deste repo (`.gitignore`), mas não faz mais parte do histórico do `tusab` (removida retroativamente via `git filter-repo` antes de tornar este repo público). Documentos relevantes: `Design System — Tusab.md` (tokens, componentes canônicos, motion, acessibilidade, anti-padrões), `Design System — Inventário de Componentes.md` (mapa de moléculas/organismos), `DesignOps — Processo de Sincronização Figma-Código.md`.
